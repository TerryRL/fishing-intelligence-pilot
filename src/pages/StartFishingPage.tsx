...
import { createWaterBody, startTrip } from '../services/dataService'
  ...
  return (
    <div className="page immersive-page">
      ...
      <section>
        ...
        <div className="choice-stack">
          {waterBodies.slice(0, 5).map((water) => (
            <button
              key={water.id}
              className={`choice-row ${waterBodyId === water.id ? 'selected' : ''}`}
              onClick={() => setWaterBodyId(water.id)}
            >
      ...
      <section>
        ...
        <div className="chip-row">
          ...
          {species.slice(0, 8).map((row) => (
            <button
              key={row.id}
              className={targetSpeciesId === row.id ? 'chip selected' : 'chip'}
              onClick={() => setTargetSpeciesId(row.id)}
            >
      ...
      <section>
        ...
        {lures.length === 0 ? (
          <div className="empty-card">
            <strong>No tackle added yet.</strong>
            <p>You can start without a lure, or add one first.</p>
            <button className="secondary-button" onClick={() => navigate('/tackle')}>Add Tackle</button>
          </div>
        ) : (
          <div className="lure-pick-grid">
            {quickLures.map((lure) => (
              <button
                className={`lure-pick ${lureId === lure.id ? 'selected' : ''}`}
                key={lure.id}
                onClick={() => setLureId(lure.id)}
              >
      ...
      <div className="sticky-action">
        <button className="primary-button" disabled={!waterBodyId || busy} onClick={() => void begin()}>
