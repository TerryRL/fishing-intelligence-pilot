-- Fishing Intelligence Pilot - Browser-Only Supabase Setup
-- PURPOSE: Run this ONE file in Supabase Dashboard > SQL Editor > New query > Run.
-- This creates the schema, indexes, triggers, Row Level Security policies,
-- private catch-photo storage bucket, and reference fish species.
-- Intended for a NEW Supabase project. Do not repeatedly run it after modifying production data.
...
-- Browser setup verification summary
select
  (select count(*) from public.species) as species_loaded,
  ...
  (select count(*) from storage.buckets where id = 'catch-photos') as catch_photo_bucket_present;
